...
def main():
    ...
    parser.add_argument(
        "--v2", action="store_true", help="Enable v2 version of embedding model"
    )
    ...
    console.print(
        f"[bold green]Processing reviews for industry: {args.industry}[/bold green]"
    )
    ...
    section_start = time.time()
    if args.v2:
        faiss_retriever = FaissRetrieverV2(
            industry=args.industry, past_excel_path=past_excel_path
        )
    else:
        faiss_retriever = FaissRetriever(
            industry_name=args.industry, past_excel_path=past_excel_path
        )
    section_end = time.time()
    ...
    (
        ...
        process_reviews_in_batches_async(
            new_reviews, faiss_retriever, llm, INDUSTRY_CATEGORIES
        )
    )
    ...
    console.print(
        f"[bold yellow]Average Total Tokens: {total_tokens / total_api_calls} tokens[/bold yellow]"
    )
    console.print(
        f"[bold yellow]Average Review Length: {average_review_length} words[/bold yellow]"
    )
    console.print(
        f"[bold magenta]Script total runtime: {total_time:.2f} seconds[/bold magenta]"
    )
