...
from app.crud.index import get_index, update_job_status
        ...
        try:
            ...
            if old_index and os.path.exists(old_index.index_path) and not replace:
                ...
                try:
                    ...
                    print("operation_success:", operation_success)
                ...
                try:
                    if old_index and replace:
                        ...
                        if not operation_success:
                            if retriever.cancel_requested:
                                ...
                                await update_job_status(
                                    ...
                                    "failed",
                                    user=user,
                                    error="Failed to create temporary index",
                                )
                            return
                        ...
                        try:
                            ...
                            try:
                                ...
                                error_msg = (
                                    f"Failed to replace index files: {str(e)}"
                                )
                                logger.error(error_msg)
                                await update_job_status(
                                    db,
                            ...
                            error_msg = (
                                f"Database error during index replacement: {str(e)}"
                            )
                            logger.error(error_msg)
                            await db.rollback()
                            await update_job_status(
                    ...
                    error_msg = f"Failed to replace/create new index: {str(e)}"
                    logger.error(error_msg)
                    await update_job_status(
                        db, job_id, "failed", user=user, error=error_msg
                    )
            ...
            try:
                ...
                error_msg = f"Database error during job finalization: {str(e)}"
                logger.error(error_msg)
                await update_job_status(
                    db, job_id, "failed", user=user, error=error_msg
                )
