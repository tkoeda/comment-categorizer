...
from utils import time_to_seconds, display_rate_limit_progress
...
class OpenAILLM:
    ...
    async def _call_api_with_retry(self, chat_params: dict):
        ...
        """
        raw_response = await self.client.chat.completions.with_raw_response.create(**chat_params)

        response = raw_response.parse()

        headers = raw_response.headers

        console.print(f"[bold yellow]Total tokens used: {response.usage.total_tokens}[/bold yellow]")
        ...
        if remaining_tokens < 4000:
            console.print(f"[red]Waiting {reset_time} seconds for token reset...[/red]")
    ...
    def _default_response(self, num_reviews: int) -> dict:
        ...
        return {"results": [{"review": i + 1, "categories": ["N/A"]} for i in range(num_reviews)]}
