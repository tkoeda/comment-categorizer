...
import { Industry } from "../../types/types.ts";
import { addIndustry, deleteIndustry } from "../../utils/utils";
...
const ManageIndustries: React.FC<ManageIndustriesProps> = ({
    industries,
    fetchIndustries,
}) => {
    ...
    const handleAddIndustry = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await addIndustry(industryName, categories);
            notifications.show({
                title: "成功",
                message: `${industryName}を追加しました`,
                color: "teal",
